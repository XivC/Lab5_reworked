...
public class ReplaceIfLower extends InterpreterCommand{
    ...
    public void execute(){
        ...
        if(!Worker.validate(new Request(new HashMap<>(){{put("salary", salary);}}, new HashMap<>()))){ outputManager.println("Неверный формат данных"); return;}
        ...
        Worker ww = (Worker)workersManager.get(new Request(new HashMap<>(){{put("key", key);}}, new HashMap<>())).attachments.get("worker").getObject();
